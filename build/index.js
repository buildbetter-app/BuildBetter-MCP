...
async function getTypeFields(typeName) {
  ...
  `;
    ...
    if (t.fields) {
        fieldsWithType.push(...t.fields.map((f) => ({ name: f.name, type: f.type, description: f.description })));
    }
    if (t.inputFields) { // For Input Object types
        fieldsWithType.push(...t.inputFields.map((f) => ({ name: f.name, type: f.type, description: f.description })));
    }
    if (t.enumValues) { // For Enum types
        fieldsWithType.push(...t.enumValues.map((ev) => ({ name: ev.name, type: { kind: "ENUM_VALUE", name: ev.name }, description: ev.description })));
    }
...
async function findSimilarFields(typeName, fieldName, maxDistance = 3) {
    try {
        ...
        const similar = fieldNames
            ...
            .sort((a, b) => (levenshteinDistance(a.toLowerCase(), fieldName.toLowerCase()) -
...
const queryTemplates = {
    ...
    "signal-by-type": {
        ...
        generateQuery: (params) => {
            ...
            const gqlVariables = [];
            const whereClauses = [`types: { type: { name: {_eq: ${type}} } }`]; // Type is directly interpolated
...
server.setRequestHandler(ReadResourceRequestSchema, async (request) => {
    ...
    if (parsedUri.protocol === "graphql:") {
        if (parsedUri.pathname === "//schema") {
            ...
            try {
                const schema = await getSchemaInfo();
                const typeDetail = schema.__schema.types.find(t => t.name === typeName);
                if (!typeDetail) {
                ...
                if (typeDetail.kind === 'OBJECT' || typeDetail.kind === 'INPUT_OBJECT') {
                    ...
                    content += `## Enum Values\n\n`;
                    const enumTypeData = schema.__schema.types.find(t => t.name === typeName && t.kind === 'ENUM');
                    if (enumTypeData && enumTypeData.enumValues && enumTypeData.enumValues.length > 0) {
                        enumTypeData.enumValues.forEach((val) => {
        ...
        try {
            ...
            if (invalidFields.length > 0) {
                ...
                if (suggestions.length > 0)
                    suggestionText = ` Did you mean one of: ${suggestions.join(', ')}?`;
            ...
            if (limit !== undefined && Number.isInteger(limit) && limit > 0)
                params.push(`limit: ${Math.min(limit, 100)}`); // Guardrail limit
...
`;
        ...
        if (smartSearchableFields.length > 0) {
            const orConditions = smartSearchableFields.map(field => `{ ${field}: {_ilike: "%${phrase}%"} }`).join(", ");
        ...
        const selectionFields = ["id", "summary", "display_ts"];
        if (extractionFields.some(f => f.name === "text"))
            selectionFields.push("text");
        if (extractionFields.some(f => f.name === "sentiment"))
            selectionFields.push("sentiment");
      ...
      query SearchExtractions {
        ...
        catch (error) { // Improvement 10: README snippet in errors
            let errorMsg = error instanceof Error ? error.message : String(error);
        ...
        if (isNaN(interviewLimit) || interviewLimit <= 0)
            interviewLimit = 1;
      ...
      `;
        ...
        if (syntaxValid) {
            ...
            while ((match = quotedEnumPattern.exec(query)) !== null) {
                potentialIssues.push(`Potential quoted enum: Found \`\"${match[2]}\"\` after \`${match[1]}\`. ` +
            ...
            for (const scalarField of commonScalarLikeFields) {
                ...
                if (subSelectionPattern.test(query)) {
                    potentialIssues.push(`Potential invalid sub-selection on '${scalarField}'. Fields like '${scalarField}' are often scalar or enum types and cannot have sub-fields (e.g., { ... }). ` +
        ...
        const patterns = [
            ...
            },
            {
                ...
                extractParams: (match) => ({ topic: match[1].trim() })
            }
        ];
            ...
            if (match) {
                ...
                if (Object.keys(params).length > 0) {
                    templateFn.parameters.forEach(pInfo => {
                        ...
                        if (params[pInfo.name] !== undefined) {
                            responseText += `  - ${pInfo.name}: ${params[pInfo.name]} (Type: ${pInfo.type}${pInfo.description ? `, ${pInfo.description}` : ''})\\n`;
                        }
                    });
                }
                else {
...
server.setRequestHandler(GetPromptRequestSchema, async (request) => {
    ...
    switch (name) {
        ...
        case "call-details": {
            const id = args.id;
            if (!id)
                throw { code: -32602, message: "Missing required argument 'id' for call-details prompt." };
        ...
        case "call-transcript": {
            const id = args.id;
            if (!id)
                throw { code: -32602, message: "Missing required argument 'id' for call-transcript prompt." };
        ...
        case "search-transcript": {
            ...
            const phrase = args.phrase;
            if (!id || !phrase)
                throw { code: -32602, message: "Arguments 'id' and 'phrase' are required for search-transcript prompt." };
            const query = `query SearchTranscriptContent {\n  interview_monologue(\n    where: {\n      text: {_ilike: "%${normalizeEnumValue(phrase)}%"},\n      interview_id: {_eq: "${id}"}\n    },\n    order_by: {start_sec: asc}\n  ) {\n    id\n    start_sec\n    end_sec\n    text\n    speaker_person { first_name, last_name } \n    interview { id name }\n  }\n}`;
            return sendUserMessage(`Search for "**${phrase}**" in the transcript of call (interview) **${id}** using:\n\n\`\`\`graphql\n${query}\n\`\`\``);
        }
        case "call-extractions": {
            const id = args.id;
            if (!id)
                throw { code: -32602, message: "Missing required argument 'id' for call-extractions prompt." };
            const typeName = args.type ? normalizeEnumValue(args.type) : undefined;
        ...
        case "feature-requests-by-date": {
            ...
            if (!startDate || !endDate)
                throw { code: -32602, message: "Arguments 'startDate' and 'endDate' are required for feature-requests-by-date prompt." };
